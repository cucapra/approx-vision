...
// This pipeline reverses the pipeline, applys green normalization, and
// approximate tone mapping with a small gamma
  ...
  for (int i=0; i<10000; i++) { //i<10000
    ...
    Func approx_tonemap("approx-tonemap");
      approx_tonemap(x,y,c) = max( min( select(
              normalize(x,y,c) < 32,  normalize(x,y,c) * 4,
              normalize(x,y,c) < 128, normalize(x,y,c) + 96,
                                      normalize(x,y,c)/4 + 192) 
                                          , 255), 0);
