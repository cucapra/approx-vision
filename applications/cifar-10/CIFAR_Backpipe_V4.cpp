...
int main(int argc, char **argv) {
  ...
  // Establish IO
  char val, label;
  fstream infile("../data-links/cifar-10-batches-bin/data_batch_4.bin");
  //fstream infile("data_batch_4_converted.bin");

  fstream outfile;
  outfile.open("data_batch_4_converted_V4.bin",fstream::out);
  ...
  for (int i=0; i<10000; i++) { //i<10000
    ...
    Func scale("scale");
      scale(x,y,c) = cast<float>(input(x,y,c));
