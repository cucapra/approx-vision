...
// This is a simple sanity check which reverses and the uses forward tone mapping
...
int main(int argc, char **argv) {
  ...
  for (int i=0; i<10000; i++) { //i<10000
    ...
    Func tonemap("tonemap");
      ...
      tonemap(x,y,c) = cast<uint8_t>(argmin( abs( rev_tone_h(c,idx2) - rev_tonemap(x,y,c) ) )[0]);
    ...
    // backward pipeline
    output = tonemap.realize(input.width(), 
                             input.height(), 
                             input.channels());
