...
int main(int argc, char **argv) {
  ...
  // Establish IO
  char val, label;
  fstream infile("/work/mark/datasets/cifar-10/cifar-10-batches-bin/data_batch_5.bin");
  //fstream infile("data_batch_1_converted.bin");

  fstream outfile;
  outfile.open("data_batch_5_converted_V9.bin",fstream::out);
  ...
  for (int i=0; i<10000; i++) { //i<10000
    ...
    Func approx_tonemap("approx-tonemap");
      approx_tonemap(x,y,c) = max( min( select(
              normalize(x,y,c) < 16,  normalize(x,y,c) * 8,
              normalize(x,y,c) < 32,  normalize(x,y,c) * 4 + 64,
              normalize(x,y,c) < 64,  normalize(x,y,c) * 1 + 160,
              normalize(x,y,c) < 128, normalize(x,y,c) / 4 + 208,
                                      normalize(x,y,c) / 8 + 224) 
                                          , 255), 0);
