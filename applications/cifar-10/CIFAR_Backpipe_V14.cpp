...
// This pipeline reverses tone mapping, and then applies an exponential tone mapping
// with a "gamma" of 1/16
...
int main(int argc, char **argv) {
  ...
  // Pre-computation for exponential tone mapping
  float gamma = 1/float(16);
  float A     = 255/float(pow(255.0,gamma));
  ...
  for (int i=0; i<10000; i++) { //i<10000
    ...
    Func exp_tonemap("tonemap");
      Expr rescaled = cast<float>(rev_tonemap(x,y,c))*256.0f;
      exp_tonemap(x,y,c) = cast<uint8_t>( max( min ( 
                             (A * ( pow( rescaled, gamma ) ))
                                               , 255), 0) );

    // Common scheduling
    rev_tonemap.reorder(c,x,y).bound(c,0,3).unroll(c);
    exp_tonemap.reorder(c,x,y).bound(c,0,3).unroll(c);

    ///////////////////////////////////////////////////////////////////////////////////////
    // CPU Schedule
    rev_tonemap.compile_jit();
    exp_tonemap.compile_jit();

    // Realization
    Image<uint8_t> output;
    // backward pipeline
    output = exp_tonemap.realize(input.width(), 
                             input.height(), 
                             input.channels());
