...
int run_image_pipeline(char* in_img_path, char* out_img_path, 
  ...
  run_image_pipeline_cv(&opencv_in_mat, cv_stages);
...
Func run_image_pipeline_rev(Func *in_func, 
  ...
  for (int i = 0; i < sizeof(*rev_stages)/sizeof(rev_stages[0]); i++) {
    ...
    switch( stage ) {
      ...
      case RevGamutMap: {
        Func rev_gamut_map_ctrl = make_rbf_ctrl_pts ( &out_func, num_ctrl_pts, rev_ctrl_pts_h, rev_weights_h );
        out_func = make_rbf_biases ( &out_func, &rev_gamut_map_ctrl, rev_coefs);
        out_func.compute_root();
        break;
      }
...
void run_image_pipeline_cv( Mat *InMat, PipelineStageCV cv_stages[] ) {
  ...
  for (int i = 0; i < sizeof(*cv_stages)/sizeof(cv_stages[0]); i++) {
...
Func run_image_pipeline_fwd(Func *in_func, 
  ...
  for (int i = 0; i < sizeof(*fwd_stages)/sizeof(fwd_stages[0]); i++) {
    ...
    switch( stage ) {
      ...
      case GamutMap: {
        Func gamut_map_ctrl = make_rbf_ctrl_pts ( &out_func, num_ctrl_pts, ctrl_pts_h, weights_h );
        out_func = make_rbf_biases ( &out_func, &gamut_map_ctrl, coefs);
        out_func.compute_root();
        break;
      }
